  ...
  if (loading || !isAuthenticated) {
    return (
        <div className="flex justify-center items-center min-h-screen bg-background">
            <Menu className="h-12 w-12 animate-spin text-primary" />
        </div>
    );
  }
            ...
            {navItems.map((item) => {
              ...
              return (
                <NextLink key={item.label} href={item.href} legacyBehavior passHref>
                  <a
                    className={cn(
                      "inline-flex items-center justify-center rounded-md px-3 py-1.5 text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:scale-[1.02] active:scale-[0.98]",
                      isActive
